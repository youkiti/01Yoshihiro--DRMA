...
# ------------------------------------------
# Excelファイルからデータを読み込む
df <- read.csv("dataset_n_SepsisSepticshock.csv")
...
######### 修正版
# 特定のdoseを指定(観察された範囲が望ましい)
newdata <- data.frame(Distribution_in_group = c(0,2, 5, 10))


# dose=2を基準に指定して予測　これは恣意的に指定して良いが、臨床的には0?
results <- predict(mod1, newdata, xref = 0, exp = TRUE)

# 結果の確認
print(data.frame(
  dose = newdata$Distribution_in_group,
  OR = results$pred,
  lower_CI = results$ci.lb,
  upper_CI = results$ci.ub
))